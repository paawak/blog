 ...
 *
 * - The transport layer aspects of all constraints supported by this provider
 * are always satisfied by all open connections and requests.
 *
 * - No combination of individual constraints supported by this provider can
...
public class Constraints {
    ...
    /**
     * indicates that this provider does not support implementing (or does not
     * understand how to implement) the transport layer aspects of satisfying a
     * given constraint
     **/
    private static final int NO_SUPPORT = 0;

    /**
     * indicates that this provider supports implementing all aspects of
     * satisfying a given constraint
     **/
    private static final int FULL_SUPPORT = 1;

    /**
     * indicates that this provider supports implementing the transport layer
     * aspects of satisfying a given constraint, but at least partial
     * implementation by higher layers is also needed in order to fully satisfy
     * the constraint
     **/
    private static final int PARTIAL_SUPPORT = 2;

    /**
     * maps constraint values that are supported to Boolean indicating whether
     * or not they must be at least partially implemented by higher layers to be
     * fully satisfied
     **/
    ...
    /**
     * maps constraint classes that are supported to Boolean indicating whether
     * or not such constraints must be at least partially implemented by higher
     * layers to be fully satisfied
     **/
    ...
    static {
        ...
        /*
         * The following classes are (trivially) supported just because
         * ClientAuthentication.YES, ServerAuthentication.YES, and
         * Delegation.YES are not supported.
         */
    ...
    private static int getSupport(InvocationConstraint c) {
        ...
        return support == null ? NO_SUPPORT : support.booleanValue() ? PARTIAL_SUPPORT : FULL_SUPPORT;
    }

    /**
     * Checks that we support at least the transport layer aspects of the given
     * requirements (and throws an UnsupportedConstraintException if not), and
     * returns the requirements that must be at least partially implemented by
     * higher layers and the supported preferences that must be at least
     * partially implemented by higher layers.
     *
     * [If this provider supported constraints whose transport layer aspects
     * were not always satisfied by open connections or requests, then we would
     * need a variant of this method that checks the given constraints against
     * an open connection or request. If this provider supported constraints
     * that could conflict with each other then (when not checking against an
    ...
    /**
     * Distills the given constraints to a form more directly usable by this
    ...
    public static class Distilled {
        ...
        public InvocationConstraints getUnfulfilledConstraints() {
            if (unfulfilledRequirements == null && unfulfilledPreferences == null) {
                ...
                return new InvocationConstraints(unfulfilledRequirements, unfulfilledPreferences);
            }
        }
        ...
        /**
         * If "isRequirement" is true, throws an UnsupportedConstraintException
         * if we do not support at least the transport layer aspects of the
         * given constraint.
         *
         * If we do support at least the transport layer aspects of the given
         * constraint, then if appropriate, adds it to the collection of
         * requirements or preferences that must be at least partially
        ...
        private void addConstraint(InvocationConstraint constraint, boolean isRequirement) throws UnsupportedConstraintException {
            if (!(constraint instanceof ConstraintAlternatives)) {
                ...
                if (support == NO_SUPPORT || (!relativeOK && constraint instanceof RelativeTimeConstraint)) {
                    if (isRequirement) {
                ...
                if (constraint instanceof ConnectionAbsoluteTime) {
                    ...
                    addConnectDeadline(((ConnectionAbsoluteTime) constraint).getTime());
                }
            } else {
                addAlternatives((ConstraintAlternatives) constraint, isRequirement);
            }
        }

        /**
         * If "isRequirement" is true, throws an UnsupportedConstraintException
         * if we do not support at least the transport layer aspects of at least
         * one of the constraints in the given alternatives.
         *
         * If we do support at least the transport layer aspects of at least one
         * of the constraints in the given alternatives, then if appropriate,
         * adds a ConstraintAlternatives of the supported alternatives to the
         * collection of requirements or preferences that must be at least
         * partially implemented by higher layers.
         *
         * If all of the supported alternatives need at least partial
         * implementation by higher layers, then adds a ConstraintAlternatives
         * with all of the supported alternatives to the unfulfilled collection
         * or preferences, because higher layers must support at least one of
         * them. But if at least one of the supported alternatives can be fully
         * satisfied by the transport layer, then add nothing to the unfulfilled
         * collection, because it is possible that higher layers need not
         * support any of them (and there is no way to express no constraint).
         *
         * The weakest connect deadline (with no deadline being the the weakest
         * possibility) is chosen among alternatives.
         **/
        private void addAlternatives(ConstraintAlternatives constraint, boolean isRequirement) throws UnsupportedConstraintException {
            ...
            for (Iterator i = alts.iterator(); i.hasNext();) {
                ...
                if (support == NO_SUPPORT || (!relativeOK && c instanceof RelativeTimeConstraint)) {
                    continue;
                }
            ...
            if (!forgetUnfulfilled && unfulfilledAlts != null) {
                if (isRequirement) {
                    ...
                    unfulfilledRequirements.add(ConstraintAlternatives.create(unfulfilledAlts));
                } else {
                    if (unfulfilledPreferences == null) {
                        unfulfilledPreferences = new ArrayList();
                    }
                    unfulfilledPreferences.add(ConstraintAlternatives.create(unfulfilledAlts));
                }
            }
        ...
        /**
         * Adds the given connect deadline to this object's state. The earliest
         * connect deadline is what gets remembered.
         **/
