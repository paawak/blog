...
class ServerInfo implements Cloneable {
    ...
    String getAuthString(String protocol, String method, String uri) {
        ...
        if (authScheme.equals("Basic")) {
            ...
            return "Basic " + enc.encode((authUser + ":" + authPassword).getBytes());
            ...
            String response = "Digest " + "username=\"" + authUser + "\", " + "realm=\"" + authRealm + "\", " + "nonce=\"" + authNonce + "\", " + "uri=\"" + uri + "\", "
                    + "response=\"" + digest + "\"";
            if (authOpaque != null) {
    ...
    private String computeDigest(String method, String uri) throws NoSuchAlgorithmException {
        ...
        MessageDigest md = MessageDigest.getInstance((authAlgorithm != null) ? authAlgorithm : "MD5");
        String hashA1 = encode(md, authUser + ":" + authRealm + ":" + authPassword);
     ...
     * values of the WWW-Authenticate, Proxy-Authenticate, Authorization-Info or
     * Proxy-Authorization-Info header lines.
     */
    private static class LineParser {
         ...
         * definitions. Throws an HttpParseException if the line contains syntax
         * errors.
         */
        ...
        /**
         * Returns all key/value entries associated with the given authorization
         * scheme in the parsed line, or null if the given scheme was not
         * described in the parsed line. Authorization scheme identifiers are
         * those which appear without a subsequent '=' or ',' character before
         * the next word; key/value entries are associated with the nearest
         * preceding scheme identifier. All key strings are converted to lower
         * case.
         */
    ...
    private PasswordAuthentication getPassword(final String protocol) {
        return (PasswordAuthentication) AccessController.doPrivileged(new PrivilegedAction() {
            public Object run() {
                ...
                return Authenticator.requestPasswordAuthentication(addr, port, protocol, authRealm, authScheme);
            }
        });
    }
}
